...
# File di log
LOG_FILE="containerInstallerLog.log"
...
if [[ ${#scripts[@]} -eq 0 ]]; then
    echo -e "\033[1;31mNessuno script trovato nella directory corrente (eccetto $CURRENT_SCRIPT).\033[0m" | tee -a "$LOG_FILE"
    exit 1
fi
...
echo -e "\033[1;32mTutti gli script sono stati processati.\033[0m" | tee -a "$LOG_FILE"
